...
class Chat extends Module {
				...
				switch($secondary) {
					case 'get-messages':
						if(isset($this->parameters['since']) {
							$logger->emit($logger::LOGGER_INFO, __CLASS__, __FUNCTION__, "Parameter passed");
					...
					case 'send-message':
						...
						// check for user GUID
						else if(!isset($this->parameters['user_guid']) || strlen($parameters['guid'] == 0) {
							$logger->emit($logger::LOGGER_WARN, __CLASS__, __FUNCTION__, "User GUID missing - cannot send message");
							$response = array(
								'response' => 'error',
						...
						// call the function
							...
							if(!$this->send_message($this->parameters['message'], $this->parameters['user_guid'])) {
								$logger->emit($logger::LOGGER_WARN, __CLASS__, __FUNCTION__, "Function call failure - cannot send message");
								$response = array(
									'response' => 'error',
									'message' => 'Server side function call failed'
								);
								break;
							}
							else {
	...
	function get_messages($since = null) {
		...
		$query = "SELECT date_time, user_guid, message, action FROM chat_messages WHERE room_guid = '{$this->room_guid}'";
	...
	function send_message($message, $user_guid) {
